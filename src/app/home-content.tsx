...
import { Landmark, FileText, Store, ChevronRight, Sparkles, ArrowRight, MapPin } from "lucide-react";
import type { HomeData, Post, Product, Village } from "@/lib/site-api";
import { decodeEscapedText, formatPopulation } from "@/lib/content";

function AnimatedNumber({ value }: { value: number }) {
  return <span className="text-3xl font-bold tabular-nums">{value.toLocaleString("zh-CN")}</span>;
}

export default function HomeContent({ initial }: { initial: HomeData }) {
    ...
    async function refresh() {
      try {
        const [statsRes, villagesRes, postsRes, productsRes] = await Promise.all([
          ...
          fetch("/api/villages?limit=6", { credentials: "include" }),
          fetch("/api/posts?limit=3", { credentials: "include" }),
          fetch("/api/products", { credentials: "include" }),
        ]);
        const [stats, villagesJson, postsJson, productsJson] = await Promise.all([
        ...
        if (cancelled) return;
        const posts = (postsJson.posts || []) as Post[];
        const products = (productsJson.products || []) as Product[];
        const villages = (villagesJson.villages || []) as Village[];
        setData({
          stats: {
            villages: stats.villages ?? villagesJson.pagination?.total ?? data.stats.villages,
            posts: stats.posts ?? postsJson.pagination?.total ?? data.stats.posts,
            products: stats.products ?? productsJson.pagination?.total ?? products.length,
          },
          villages: villages.slice(0, 6),
  ...
  const { stats, villages, posts, products } = data;
    ...
    <>
            ...
            <Link href="/place/">
              <Card className="bg-gradient-to-br from-blue-600 to-blue-700 text-white border-0 shadow-lg hover:shadow-xl transition-shadow">
                <CardContent className="flex items-center gap-4 p-6">
                  ...
                  <div>
                    <AnimatedNumber value={stats.villages} />
                    <div className="text-blue-100 text-sm">村庄名录</div>
                  </div>
                </CardContent>
              </Card>
            </Link>
      ...
      <section className="py-12 md:py-20 container">
        <div className="flex items-end justify-between mb-6 md:mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold">村庄名录精选</h2>
            <p className="text-muted-foreground mt-1 text-sm md:text-base">探寻滨州村庄的根脉与变迁</p>
          </div>
          <Link href="/place/">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {villages.map((v) => (
            <Link key={v.id} href={`/place/${v.id}/`} className="block h-full">
              <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-br from-slate-50 to-white flex flex-col h-full">
                <CardContent className="p-5 md:p-6 space-y-3 md:space-y-4 flex flex-col flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {v.township && <Badge variant="outline" className="text-xs">{v.township}</Badge>}
                  </div>
                  <h3 className="text-lg md:text-xl font-semibold group-hover:text-blue-600 transition-colors line-clamp-2">
                    {v.name}
                  </h3>
                  <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed flex-1 whitespace-pre-line">
                    {decodeEscapedText(v.history || v.evolution) || "该村历史沿革正在整理中。"}
                  </p>
                  <span className="inline-flex items-center text-sm text-blue-600 hover:text-blue-700 gap-1 mt-auto pt-2">
                    查看详情 <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" />
                  </span>
                </CardContent>
              </Card>
            </Link>
            ...
            <Link key={post.id} href={`/blog/${post.slug}/`} className="block h-full">
              <Card className="group hover:shadow-xl transition-all duration-300 overflow-hidden border-0 bg-gradient-to-br from-slate-50 to-white flex flex-col h-full">
                ...
                <CardContent className="p-5 md:p-6 space-y-3 flex flex-col flex-1">
                  ...
                  <span className="inline-flex items-center text-sm text-blue-600 gap-1 mt-auto pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {products.map((p) => (
              <Link key={p.id} href={`/product/${p.id}/`} className="block h-full">
                <Card className="group hover:shadow-xl transition-all duration-300 overflow-hidden border-0 flex flex-col h-full">
                  <CardContent className="p-5 md:p-6 space-y-3 flex flex-col flex-1">
